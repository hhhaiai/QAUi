...
public class LogsCheckLogsUI extends JFrame {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1826089092834096113L;
	Logger logger = LoggerFactory.getLogger(LogsCheckLogsUI.class);
	...
	LogsCheckLogs logsCheckLogs = new LogsCheckLogs();
	 ...
	 * Create the frame.
	 */
	public LogsCheckLogsUI(String udid) {
		this.udid = udid;
		...
		btnStart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (isstart == false) {
					...
					logsCheckLogs.run(udid, textArea_Inputcmd.getText(), textAreaUI.getJTextArea());
					btnStart.setText("停止");
		...
		// 命令输入框
		textArea_Inputcmd = new JTextArea(MainRun.paramsBean.getLogs_inputcmd());
		...
		btn_Highlight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				LogsHighlightUI logsHighlightUI = new LogsHighlightUI(logsCheckLogs);
	...
	public void saveParams() {
		MainRun.paramsBean.setLogs_inputcmd(textArea_Inputcmd.getText());
		MainRun.xmlOperationUtil.XMLChanger("Logs_inputcmd", textArea_Inputcmd.getText());
	}
}
