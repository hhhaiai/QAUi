...
public class Tailer implements Runnable {
	 ...
	 *            milliseconds.
	 * @param end
	 *            Set to true to tail from the end of the file, false to tail from
	...
	public void run() {
		...
		try {
			...
			while (isRun()) {
				...
				if (length < position) {
					// logger.info(String.format("rotated, legth=%s, position=%s", length,
					// position));
					// File was rotated
	...
	public boolean isFileNewer(File file, long data) {
		if (file != null && file.exists() && file.lastModified() > data) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isRun() {
		return run && !listener.isCancelled();
	}
	...
	private long readLines(RandomAccessFile reader) throws IOException {
		...
		while (isRun() && (line = reader.readLine()) != null) {
			listener.handle(new String(line.getBytes("ISO-8859-1"), "UTF-8"));
			// pos += line.length();
		}
		...
		// while (isRun() && ((num = reader.read(inbuf)) != -1)) {
		// for (int i = 0; i < num; i++) {
		// byte ch = inbuf[i];
		// switch (ch) {
		// case '\n':
		// seenCR = false; // swallow CR before LF
		// listener.handle(new String(sb.toString().getBytes("ISO-8859-1"), "UTF-8"));
		// sb.setLength(0);
		// rePos = pos + i + 1;
		// break;
		// case '\r':
