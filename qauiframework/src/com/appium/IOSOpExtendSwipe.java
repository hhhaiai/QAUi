...
public class IOSOpExtendSwipe extends BaseOpExtendSwipe {
	Logger logger = LoggerFactory.getLogger(IOSOpExtendSwipe.class);
	IOSDriver<WebElement> driver;
	IOSShot Shot;
	IOSOp Op;

	public IOSOpExtendSwipe(Object driver, Object Shot, SceneLogUtil oplog, IOSOp iosOp, Translation translation) {
		super(oplog, translation);
		// TODO Auto-generated constructor stub
		this.driver = (IOSDriver<WebElement>) driver;
		this.Shot = (IOSShot) Shot;
		this.Op = iosOp;
	...
	@Override
	public ElementIOS scrolldownTo(By by, int scrollnum) {
		// TODO Auto-generated method stub
	...
	@Override
	public ElementIOS scrollupTo(By by, int scrollnum) {
		// TODO Auto-generated method stub
	...
	@Override
	public ElementIOS scrollleftTo(By by, int scrollnum) {
		// TODO Auto-generated method stub
	...
	@Override
	public ElementIOS scrollrightTo(By by, int scrollnum) {
		// TODO Auto-generated method stub
	...
	@Override
	protected ElementIOS scrollTo(int startx, int starty, int endx, int endy, By by, int scrollnum, String direction) {
		String Text = translation.getName(by);
		if (scrollnum < 0)
			scrollnum = 10;
		oplog.logInfo("开始:向" + direction + "翻页寻找:" + Text);
		ElementIOS element = null;
	...
	@Override
	protected ElementIOS scroll2Target(String direction, int startx, int starty, int endx, int endy, By by,
			int scrollnum, String Text, int minlimit, int maxlimit) {
		ElementIOS element = scroll2TargetCore(direction, startx, starty, endx, endy, by, scrollnum, Text, minlimit,
				maxlimit);
		// 为了解决有时候元素被滑过了而造成的无法找到元素,相反方向折半滑动
		if (!element.exist()) {
	...
	@Override
	protected ElementIOS scroll2TargetCore(String direction, int startx, int starty, int endx, int endy, By by,
			int scrollnum, String Text, int minlimit, int maxlimit) {
		if (scrollnum < 0)
			scrollnum = 10;
		int count = 0;
		ElementIOS element = new ElementIOS(null, by, Text, Op.getElementMap());
		do {
			count++;
			WebElement webElement = null;
			try {
				webElement = new IOSDriverWait(driver, 3).until(new IOSExpectedCondition<WebElement>() {
					@Override
			...
			if (webElement == null) {
				...
				if (direction.equals(LEFT) || direction.equals(RIGHT)) {
					if (point.getX() + dimension.getWidth() / 2 > maxlimit
							|| point.getX() + dimension.getWidth() / 2 < minlimit) {
	...
	public boolean swipeSeekbar(By by, String direction, int startadjust, int addx) {
		...
		String Text = translation.getName(by);
		ElementIOS element = Op.findElement(by);
		if (element.exist()) {
	...
	@Override
	public ElementIOS dragSeekbarTo(By by, String direction, int startadjust, int endadjust, By targetby,
			int scrollnum) {
		// TODO Auto-generated method stub
		startadjust = Op.X_multiple(startadjust);
		endadjust = Op.X_multiple(endadjust);
		...
		ElementIOS element = Op.findElement(by);
		ElementIOS targetelement = new ElementIOS(null, by, Text, Op.getElementMap());
		if (element.exist()) {
			WebElement webElement = element.getWebElement();
			Point point = webElement.getLocation();
			Dimension dimension = webElement.getSize();
	...
	public List<String> dragSeekbarTogetItems(By by, String direction, By item, int startadjust, int endadjust) {
		...
		do {
			nonew = true;
			for (ElementIOS element : Op.findElement(by).findElements(item)) {
	...
	protected void swipe(int startx, int starty, int endx, int endy, int duration) {
		...
		// if (difx >= device_width) {
		// difx = device_width - 1;
		// } else if (difx <= -device_width) {
		// difx = -device_width + 1;
		// } else if (difx == 0) {
		// difx = 1;
		// }
		// if (dify >= device_hight) {
		// dify = device_hight - 1;
		// } else if (dify <= -device_hight) {
		// dify = -device_hight + 1;
		// } else if (difx == 0) {
		// dify = 1;
		// }
